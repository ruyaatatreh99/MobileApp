...
userlogin=()=>{
   ...
   fetch('http://192.168.1.113/project/login.php',{
        method:'POST',
        header:{
    ...
    .then((responseJason)=>{
         ...
         if(responseJason === 'Enter email and password')alert(responseJason);
          ...
          AsyncStorage.setItem('bio',JSON.stringify(responseJason.bio));
          AsyncStorage.setItem('phonenumber',JSON.stringify(responseJason.phonenumber));
          AsyncStorage.setItem('address',JSON.stringify(responseJason.address));
          AsyncStorage.setItem('cost',JSON.stringify(responseJason.cost));
          AsyncStorage.setItem('image',JSON.stringify(responseJason.image));
          AsyncStorage.setItem('total',JSON.stringify(responseJason.total));
          AsyncStorage.setItem('usertotal',JSON.stringify(responseJason.total));
          AsyncStorage.setItem('cost',JSON.stringify(responseJason.cost));
  ...
  render(){
    ...
    const pressHandlerForget=()=>{
      this.props.navigation.navigate('ForgetPass');
    }

    const pressHandlerNew=()=>{
    ...
    return (
        ...
        <View style={styles.container}>
          ...
          <Text style={styles.Titles}>Welcome</Text>
            ...
            <Button  
              onPress={()=> pressHandlerForget()}
              title='Forget your Password?!' 
              color='#dc143c'
              />

           <Button  
              onPress={()=> pressHandlerNew()}
